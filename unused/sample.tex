...
\begin{remarkbox}
	\begin{enumerate}
		\item 在实数轴上，表示两点距离的符号是求两点间的绝对值，如$|x - y|$；在$\mathbb{R}^n$维空间上，求两点距离的符号是$\lVert  \rVert$，如$\lVert x-y \rVert$。
		\item 	三角不等式的几何意义：有三个点$x,y,z \in \mathbb{R}^n$，设向量$\vec{x} = x - y,\vec{y} = y - z$，三角不等式就是这三点围成的三角形中，两边之和大于第三边。
	\end{enumerate}
\end{remarkbox}
...
\section{不同的数学符号字体}
\begin{enumerate}
	\item mathcal：花体、书法字体（calligraphy），如：
		$$
		...
		$$
			\mathcal N
		$$
	\item mathbb：黑板粗体（blackboard bold）：
		$$
			\mathbb{ABCDEFGHIJKLMNOPQRSTUVWXYZ}
		$$
	\item mathbf：粗体、黑体（boldface）
		$$
...
\section{字体示例}
   %字体族设置（罗马字体、无衬线字体、打字机字体）
    ...
    %字体形状（直立、斜体、伪斜体、小型大写）
    \textup{直立（Upright Shape）}  \textit{斜体（Italic Shape）}
    \textsl{伪斜体（Slanted Shape）}  \textsc{小型大写（Small Caps Shape）}
...
x & = x_0 + t \cos \alpha, \\
y & = y_0 + t \sin \alpha \\
z & = f(x_0 + t \cos a, y_0 + t \sin \alpha)
\end{aligned}
\right.
$$
...
\section{一些特殊的符号}
\begin{enumerate}
	\item $ \ell ~ \boldsymbol\ell $
\end{enumerate}
\section{画图}
\subsection{树形图}
主要参数：
\begin{enumerate}
	\item thick,scale是整张树形图的大小，可以在0~1内调整树形图的大小
	\item every node/.style=\{scale=0.8\}是每个节点文字的大小，可以修改调整节点文字的大小。
	\item child [missing] \{\}充当间隔符号，隔开各个子树，可以增减其个数调整子树间距。
	\item ['grow'=right]：'grow'=上下左右，可以调整树形图开口方向
\end{enumerate}
...
\begin{tikzpicture}   %创建环境
            [thick,scale=0.6, every node/.style={scale=0.8}]
...
\section{表格参数}
花括号中的 “\{c|c\}” 表示文本在列中的位置。下表总结了位置参数及其说明：
\begin{enumerate}
	\item c 	将文本置于中间
	\item l 	将文本左对齐
	\item r 	将文本右对齐
...
multirow 采用以下参数 “$\textbackslash$multirow{行的数量}{宽度}{文本}”：
...
\subsubsection{多列表格}
multicolumn 参数是 “$\textbackslash$multicolumn\{列的数量\}\{单元格选项\}\{位置\}\{文本\}”：
