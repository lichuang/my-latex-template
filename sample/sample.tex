...
\usepackage{mdframed}
\usepackage{subcaption,booktabs} % subtable
...
% Set A
\node[set,label={135:$A$}] (A) at (0.8,1) {};
 
% Set B
\node[set,label={45:$B$}] (B) at (1.1,1) {};
 
% Intersection
\begin{scope}
    \clip (0,0) circle(1.8cm);
    \clip (1.8,0) circle(1.8cm);
    \fill[green](0,0) circle(1.8cm);
\end{scope}
 
% Circles outline
\draw (0,0) circle(1.8cm);
\draw (1.8,0) circle(1.8cm);
 
% Set intersection label
\node at (0.9,0) {内连接};
\end{tikzpicture}
\label{pic:inner_join}
\caption{内连接}
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
\centering
...
% Circles outline
\draw (0,0) [fill=green]circle(1.8cm);
\draw (1.8,0) circle(1.8cm);
...
% Circles outline
\draw (1.8,0) [fill=green] circle(1.8cm);
\draw (0,0) circle(1.8cm);

% Set intersection label
\node at (0.9,0) {右连接};
\end{tikzpicture}
\label{pic:right_join}
\caption{右连接}
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
\centering
...
\begin{minipage}[t]{0.5\textwidth}
\begin{flushleft}
\textup{record}执行规则 \fbox{$\symrm{t \rightarrow t_1^{'}}$}：
\begin{align*}
...
\begin{minipage}[t]{0.5\textwidth}
\begin{flushleft}
\textup{record}类型规则 \fbox{$\symrm{\Gamma \vdash t \colon T}$}：
\begin{align*}
